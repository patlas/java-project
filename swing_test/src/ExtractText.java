...
public class ExtractText {
	public static void main(String[] args) throws Exception {
		...
		List<Element> linkElements2=source.getAllElementsByClass("ellipsis");
		for (Element linkElement2 : linkElements2) {
			//String href=linkElement2.getAttributeValue("href");
			//if (href==null) continue;
			// A element can contain other tags so need to extract the text from it:
			String label=linkElement2.getContent().getTextExtractor().toString();
			
			/*List<Attribute> Atributes = linkElement2.getAttributes();
			for(Attribute Atribute  : Atributes )
			
			 Atribute.getValue().toString();*/
				
			List<Element> AAA = linkElement2.getAllElements(HTMLElementName.A);
			String x = AAA.get(0).getAttributeValue("data-time");
			
			String timing = linkElement2.getAllElements(HTMLElementName.A).get(0).getAttributeValue("data-time");
			
//			System.out.println(label+" <"+href+'>');
			System.out.println(label + " " + timing + " " + x);
		}
